...
void KeyFinder::setTargets(std::string targetsFile) {
	...
	Logger::log(LogLevel::Info,
			CommonUtils::formatThousands(_targets.size())
					+ " address(es) loaded ("
					+ CommonUtils::format("%.1f",
							(double)(sizeof(KeySearchTarget) * _targets.size())
									/ (double)(1024 * 1024)) + "MB)" + "\n"
											+ CommonUtils::formatThousands(invalidAddressCount)
											+ " address(es) ignored");
...
void KeyFinder::setTargetsOnDevice() {
	...
	for (std::set<KeySearchTarget>::iterator i = _targets.begin();
			i != _targets.end(); ++i) {
	...
	while (_running) {
		...
		if (_device->getResults(results) > 0) {
			for (unsigned int i = 0; i < results.size(); i++) {
				...
				info.address = Address::fromPublicKey(results[i].publicKey,
		...
		if (!_randomMode
				&& (_device->getNextKey().cmp(_endKey) >= 0
						|| _device->getNextKey().cmp(_startKey) < 0)) {
