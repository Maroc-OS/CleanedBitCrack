 ...
 * Base point X
 */
__constant unsigned int GX[8] = {0x79BE667E, 0xF9DCBBAC, 0x55A06295,
 ...
 * Base point Y
 */
__constant unsigned int GY[8] = {0x483ADA77, 0x26A3C465, 0x5DA4FBFC,
 ...
 * Group order
 */
__constant unsigned int N[8] = {0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFE,
                                0xBAAEDCE6, 0xAF48A03B, 0xBFD25E8C, 0xD0364141};

/**
 * Prime modulus 2^256 - 2^32 - 977
 */
__constant unsigned int P[8] = {0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
                                0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFE, 0xFFFFFC2F};

#ifdef DEVICE_VENDOR_INTEL
// Intel devices have a mul_hi bug
FORCE_INLINE unsigned int mul_hi977(unsigned int x) {
...
FORCE_INLINE void madd977(unsigned int *high, unsigned int *low, unsigned int *a,
                    unsigned int *c) {
  *low = *a * 977;
...
#define isInfinity256k(a)                                                      \
  ((a[0] == 0xffffffff) && (a[1] == 0xffffffff) && (a[2] == 0xffffffff) &&     \
   (a[3] == 0xffffffff) && (a[4] == 0xffffffff) && (a[5] == 0xffffffff) &&     \
   (a[6] == 0xffffffff) && (a[7] == 0xffffffff))

#define greaterOrEqualToP(a) (a[6] >= P[6] || a[7] >= P[7])
...
void multiply256(const unsigned int x[8], const unsigned int y[8],
                 unsigned int out_high[8], unsigned int out_low[8]) {
...
void mulModP(unsigned int a[8], unsigned int b[8],
             unsigned int product_low[8]) {
...
void addModP256k(const unsigned int a[8], const unsigned int b[8],
                 unsigned int c[8]) {
  __private unsigned int borrow = 0;
  __private unsigned int carry = 0;
...
void doBatchInverse256k(unsigned int x[8]) { invModP256k(x); }

void beginBatchAdd256k(const uint256_t px, const uint256_t x,
                       __global uint256_t *chain, const int i,
                       const int batchIdx, uint256_t *inverse) {
...
void beginBatchAddWithDouble256k(const uint256_t px, const uint256_t py,
...
void completeBatchAdd256k(const uint256_t px, const uint256_t py,
                          __global uint256_t *xPtr, __global uint256_t *yPtr,
                          const int i, const int batchIdx,
                          __global uint256_t *chain, uint256_t *inverse,
                          uint256_t *newX, uint256_t *newY) {
  __private int gid = get_local_size(0) * get_group_id(0) + get_local_id(0);
  __private int dim = get_global_size(0);
  __private uint256_t x = xPtr[i];
...
void completeBatchAddWithDouble256k(const uint256_t px, const uint256_t py,
                                    __global const uint256_t *xPtr,
                                    __global const uint256_t *yPtr, const int i,
                                    const int batchIdx,
                                    __global uint256_t *chain,
                                    uint256_t *inverse, uint256_t *newX,
                                    uint256_t *newY) {
  __private int gid = get_local_size(0) * get_group_id(0) + get_local_id(0);
  __private int dim = get_global_size(0);
...
unsigned int readWord256k(__global const uint256_t *ara, const int idx,
                          const int word) {
