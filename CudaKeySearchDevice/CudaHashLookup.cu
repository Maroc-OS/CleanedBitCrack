...
void CudaHashLookup::initializeBloomFilter64(const std::vector<struct hash160>& targets, unsigned int* filter, unsigned long long mask)
{
    for (unsigned int k = 0; k < targets.size(); k++) {
        ...
        idx[0] = ((unsigned long long)hash[0] << 32u | hash[1]) & mask;
        idx[1] = ((unsigned long long)hash[2] << 32u | hash[3]) & mask;
        idx[2] = ((unsigned long long)(hash[0] ^ hash[1]) << 32u | (hash[1] ^ hash[2])) & mask;
        idx[3] = ((unsigned long long)(hash[2] ^ hash[3]) << 32u | (hash[3] ^ hash[4])) & mask;
        idx[4] = ((unsigned long long)(hash[0] ^ hash[3]) << 32u | (hash[1] ^ hash[3])) & mask;
